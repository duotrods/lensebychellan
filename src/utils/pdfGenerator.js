...
export const generateReportPDF = async (
  ...
  switch (reportType) {
    ...
    case "cctv-check":
      ...
      if (
        (!filterSchemeId || filterSchemeId === "Gallows") &&
      ...
      // Costain Simister Island Section - only show if no filter OR filter matches CSI
      if (
        (!filterSchemeId || filterSchemeId === "SimisterIsland") &&
        (report.csi || report.csiComments)
      ) {
        yPosition += 3;
        doc.setFillColor(245, 245, 245);
        doc.rect(margin, yPosition - 3, contentWidth, 10, "F");
        ...
        doc.setFontSize(11);
        doc.setFont("helvetica", "bold");
        doc.text("Costain Simister Island", margin + 3, yPosition + 3);
        yPosition += 12;
