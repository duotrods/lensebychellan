...
import { Upload, Video, X, Eye } from "lucide-react";
...
const BodyCamUploadPage = () => {
  ...
  const loadUploads = async () => {
    ...
    try {
      ...
      const bodyCamUploads = await staffService.getBodyCamUploads(null);

      const isDemo = isDemoUser(userProfile);
      const filteredUploads = bodyCamUploads.filter((upload) => {
      ...
      console.error("Failed to load body cam uploads:", error);
  ...
  const uploadFile = async () => {
    ...
    try {
      const key = `bodycam-uploads/${userProfile.uid}/${Date.now()}_${selectedFile.name}`;
      ...
      await staffService.submitBodyCamUpload(
        {
          ...uploadForm,
      ...
      toast.success("Body cam footage uploaded successfully!");
    ...
    <StaffSidebarLayout>
      <div>
        <div className="mb-6">
          ...
          <p className="text-gray-600">Upload and manage body cam footage</p>
        </div>
              ...
              <div className="mb-6">
                ...
                >
                  <input
                    ...
                    className="hidden"
                    id="bodycam-video-upload"
                    disabled={uploadingFile}
                  />
                  <label htmlFor="bodycam-video-upload" className="cursor-pointer">
                    <Upload
...
export default BodyCamUploadPage;
