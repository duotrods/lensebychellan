...
const ThirdPartyChartsPage = () => {
  ...
  // dashboard and ClientChartsPage use — one document read on a cache hit
  // instead of scanning every incident in the window, and the Firestore-side
  // cache (15 min TTL) is shared across every viewer of this scheme+date
  // combination. Replaces the old unbounded raw-document fetch, which had no
  ...
  const statsQuery = useQuery({
    ...
    // — a scheme+date combo already viewed there (or on ClientChartsPage) is
    // served from the shared server-side cache at no extra cost.
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      await exportChartsToPDF({
        reportTitle: 'Third Party Charts & Analytics',
        ...
        statsText,
        charts,
        fileName: `third_party_charts_${selectedScheme.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`,
      });
